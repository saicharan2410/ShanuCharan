...
jQuery(document).ready(function($) {
	...
	function init() {
		var myLatlng = new google.maps.LatLng(17.317405, 78.552023);
		var maptooltipbold = 'SS Grand Convention';
		var maptooltip = 'Hastinapuram, Hyderabad, Telangana, India';
		//---------------------------------------------------------//
		var mapOptions = {
			zoom: 17,
			scrollwheel: false,
...
jQuery(document).ready(function($){
	...
	//var weddingDate = new Date(weddingDate.getFullYear(), 12 - 1, 09);
	weddingDate = new Date(weddingDate.getFullYear() + 1, 1 - 1, 1);
	$(".countdown").countdown({
		until: new Date(2016, 12-1, 9, 8, 31),
		padZeroes: true,
		format: 'ODHMS'
	});
});
...
jQuery(document).ready(function($){
	...
	"&copy;  " + new Date().getFullYear() + " <a href='https://www.facebook.com/pathyam.charan'>Sai Charan Pathyam</a>. All rights reserved.";
});
